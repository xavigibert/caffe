...
void convert_dataset(int num_files, char** image_filename,
  ...
  // Save a list of file_idx/file_offset pairs for each class
  std::vector<std::pair<int,uint64_t> > samples[2];
  ...
  for (int file_idx = 0; file_idx < num_files; file_idx++) {
    ...
    for (int item_id = start_id; item_id < end_id; ++item_id) {
      ...
      CHECK_GE(label, 0);
      CHECK_LE(label, 1);
      samples[label].push_back(std::make_pair<int,uint64_t>(file_idx,src_offset));
    }
  }
  }
  for (int i = 0; i < std::max(samples[0].size(),samples[1].size()); i++) {
    int r = rand() % 2;
    for (int j = 0; j < 2; j++) {
      int label = r == 0 ? j : 1-j;
      if (samples[label].size() == 0)
        continue;
      std::pair<int,uint64_t> d = samples[label][i%samples[label].size()];
      datum.set_file_idx(d.first);
      datum.set_src_offset(d.second);
